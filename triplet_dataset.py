...
class TrainDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 获取正样本
        pos_photo = Image.open(photo_path)
        # 如果图片是单通道的，转换成三通道
        if len(pos_photo.split()) == 1:
            pos_photo = pos_photo.Merge("RGB", (pos_photo, pos_photo, pos_photo))
        pos_photo = self.transform(pos_photo)

        # 负样本是随机选择的,不同类别的图片
        neg_photo_name = random.choice(self.photo_list)
        while neg_photo_name.split('.')[0] == photo_label:
            neg_photo_name = random.choice(self.photo_list)
        neg_photo_label = neg_photo_name.split('.')[0]
        neg_photo_path = os.path.join(self.photo_root, neg_photo_name)
        neg_photo = Image.open(neg_photo_path)
        # 如果图片是单通道的，转换成三通道
        if len(neg_photo.split()) == 1:
            neg_photo = neg_photo.Merge("RGB", (neg_photo, neg_photo, neg_photo))
        neg_photo = self.transform(neg_photo)  
        ...
        return pos_photo, sketch, neg_photo
