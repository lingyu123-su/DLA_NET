...
from triplet_dataset import TrainDataset, TestPhDataset, TestSkDataset
...
def train():
    ...
    for i in range(epoch + 1, opt.epoch + 1):
        ...
        for pos_photo, sketch, neg_photo in tqdm_batch:
            # to device
            ...
            # get local features
            pos_photo_feature = local_photo_feature_extractor(pos_photo)
            #print(pos_photo_feature.shape)
            local_sketch_feature = local_sketch_feature_extractor(sketch)
            #print(local_sketch_feature.shape)
            neg_photo_feature = local_photo_feature_extractor(neg_photo)
            #print(neg_photo_feature.shape)

            # get distance
            pos_dis = match(pos_photo_feature, local_sketch_feature)
            neg_dis = match(neg_photo_feature, local_sketch_feature)

            # get loss
            loss = torch.nn.functional.relu(pos_dis - neg_dis + opt.margin)
            loss = torch.mean(loss)
            loss_tracker.update(loss.item(), pos_photo.shape[0])
